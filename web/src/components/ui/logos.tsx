...
type SimpleIcon = { title: string; path: string; viewBox?: string; fillRule?: "evenodd" };
...
// VS Code is absent from simple-icons (trademark); silhouette from its official mark (svgl).
const siVscode: SimpleIcon = {
  title: "VS Code",
  viewBox: "0 0 100 100",
  fillRule: "evenodd",
...
function Glyph({ icon, className = "size-4" }: { icon: SimpleIcon; className?: string }) {
  return (
    <svg role="img" viewBox={icon.viewBox ?? "0 0 24 24"} fill="currentColor" className={className} aria-label={icon.title}>
...
const CONNECTABLE: readonly BrandName[] = ["Claude", "Cursor", "Codex", "Gemini", "VS Code", "Zed", "Windsurf"];
